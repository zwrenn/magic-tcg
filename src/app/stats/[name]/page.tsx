...
export default async function PlayerStatsPage({
  ...
  const [totals, { items: rows }, trophies] = await Promise.all([
    collectionTotals(who.id),
    searchUserCollection(who.id, { limit: 100000 }),
    getTrophies(who.id),
  ]);
  ...
  for (const r of rows) {
    byColor.set(
      colorBucket(r.colorIdentity),
      (byColor.get(colorBucket(r.colorIdentity)) ?? 0) + r.quantity
    );
    byType.set(
      typeBucket(r.typeLine),
      (byType.get(typeBucket(r.typeLine)) ?? 0) + r.quantity
    );
    if (r.setCode) sets.add(r.setCode);
  }

  const colorRows = COLOR_BUCKETS.map((c) => ({
    label: c,
    value: byColor.get(c) ?? 0,
  })).filter((r) => r.value > 0);
  const typeRows = TYPE_BUCKETS.map((t) => ({
    label: t,
  ...
  return (
    <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-8">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat
          label="Est. value"
          value={`$${totals.valueUsd.toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
          accent
        />
      </div>
                ...
                >
                  {r.image ? (
                    ...
                    <img
                      ...
                      alt={r.name}
                      loading="lazy"
                      className="aspect-[488/680] w-full rounded-lg border border-border object-cover"
                    />
                  ) : (
                    <span className="flex aspect-[488/680] w-full items-center justify-center rounded-lg border border-border bg-surface-2 p-2 text-center text-xs text-muted">
                      {r.name}
                    </span>
                  )}
                </CardZoomButton>
          ...
          {trophies.map((t) => (
            ...
            >
              ...
              <div className="min-w-0">
                <div
                  className={`truncate font-semibold ${t.earned ? 'text-[#3a3358]' : 'text-foreground'}`}
                >
                  {t.name}
                </div>
                {t.earned ? (
                  <div className="text-xs font-semibold text-[var(--green-deep)]">
                    Earned!
                  </div>
                ) : (
                  <>
...
function Stat({
  ...
  return (
    <div className="card p-4">
      ...
      <div
        className={`mt-1 text-2xl font-semibold ${accent ? 'text-accent' : ''}`}
      >
        {value}
      </div>
    </div>
