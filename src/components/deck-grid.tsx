    ...
    <section>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h2 className="t-hero text-3xl">
          Your Decks{' '}
          <span className="lcd lcd-green ml-1 align-middle text-xl">
            ×{decks.length}
          </span>
        </h2>
        <div className="flex items-center gap-1">
          ...
          {(['recent', 'az', 'owned'] as Sort[]).map((s) => (
            <button
              ...
              className={`rounded-full border-[3px] px-3 py-1 text-xs font-bold tracking-wide uppercase transition ${
                sort === s
                  ? 'border-white bg-[linear-gradient(180deg,#b48bff,#9b6cff)] text-white shadow-[0_3px_0_rgba(122,79,224,0.5)]'
                  : 'border-[var(--border)] bg-surface text-muted hover:-translate-y-0.5 hover:border-[var(--purple)]'
              }`}
            >
              {s === 'az' ? 'A–Z' : s}
            </button>
      ...
      <ul className="grid gap-4 sm:grid-cols-2 xl:grid-cols-2">
        ...
        <li>
          ...
          >
            ...
            <span className="t-hero text-xl text-[var(--purple-deep)]">
              Build a New Deck
            </span>
            <span className="text-sm text-muted">
              Paste · Archidekt · Commander
            </span>
          </Link>
        </li>
      </ul>
  ...
  return (
    <div className="card hover-pop flex h-full flex-col overflow-hidden !p-0">
      ...
      >
        ...
        <ColorDots
          identity={d.colors}
          className="relative z-[5] shrink-0 text-base"
        />
      </div>
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        <div className="mt-2 flex items-baseline justify-between gap-2">
          <span className="truncate text-sm text-muted italic">
            {d.commander ?? '—'}
          </span>
          <span className="pixel shrink-0 text-xs text-subtle uppercase">
            {d.source}
          </span>
        </div>
